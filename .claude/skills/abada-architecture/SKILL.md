...
| `service` (in progress) | metadata in (`metadata::incoming`), a successful response out (`response::Response::success`), the call itself (`call::unary`, in-process only — `codec::DynamicCodec`), and the `tower::Service` (`Gateway`, a `Router<Registration>` of hand-registered RPCs), all done narrow-scope; proxy, `response_body`, NDJSON streaming (planned) |
